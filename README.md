...
- Automated PubMed literature evidence can now be fetched with `python -m q_ai_drug.cli build-literature-evidence --config configs/cancer_targets.yaml --out outputs/cancer_proof_v1/literature`. It writes target-level context, query-audit, and summary CSV/JSON artifacts, and reports/candidate evidence documents label those records as literature context only, not candidate validation.
